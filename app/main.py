...
def show_agents_tab():
    ...
    for agent in agents:
        with st.expander(f"🤖 {agent['name']}"):
            ...
            with col1:
                if st.button(f"Configurar {agent['name']}", key=f"config_{agent['name']}"):
                    st.info(f"Configuração do agente {agent['name']} em desenvolvimento")
...
def show_crews_tab():
    ...
    existing_crews = [
        {"name": "Crew de Pesquisa", "agents": ["Pesquisador", "Analista"], "status": "Ativa"},
        {"name": "Crew de Conteúdo", "agents": ["Escritor", "Revisor"], "status": "Ativa"},
