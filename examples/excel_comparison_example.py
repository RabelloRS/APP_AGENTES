"""
Exemplo prático de comparação de planilhas Excel usando o APP_AGENTES
...
def create_sample_excel_files():
    ...
    materials_old = {
        ...
        'Unidade': ['sacos', 'm³', 'm³', 'kg', 'un', 'sacos', 'galões', 'un', 'un', 'un'],
        'Preço_Unit': [25.50, 45.00, 65.00, 3.20, 0.85, 18.00, 45.00, 2.50, 150.00, 1.20]
    }
    ...
    materials_new = {
        ...
        'Unidade': ['sacos', 'm³', 'm³', 'kg', 'un', 'sacos', 'galões', 'un', 'un', 'un', 'sacos', 'galões'],
        'Preço_Unit': [26.00, 47.00, 68.00, 3.25, 0.88, 19.00, 48.00, 2.60, 155.00, 1.25, 28.00, 52.00]
    }
...
def demonstrate_excel_analysis():
    ...
    print("🚀 Demonstração do MVP - Análise de Planilhas Excel")
    print("=" * 60)
    ...
    if validation1["is_valid"] and validation2["is_valid"]:
        print(f"✅ Arquivo 1: {validation1['total_rows']} linhas, {validation1['total_columns']} colunas")
        print(f"✅ Arquivo 2: {validation2['total_rows']} linhas, {validation2['total_columns']} colunas")
    else:
    ...
    try:
        analysis_results = analyze_excel_similarity(file1_path, file2_path, "Material", "Material")
        
        print(f"✅ Análise concluída!")
        print(f"📈 Score médio de similaridade: {analysis_results['similarity_analysis']['average_score']:.1f}%")
        print(f"🎯 Itens com alta similaridade (≥80%): {analysis_results['similarity_analysis']['high_similarity_count']}")
        ...
        for i, (original, match_info) in enumerate(sorted_matches[:3], 1):
            print(f"   {i}. '{original}' → '{match_info['match']}' (Score: {match_info['score']:.1f}%)")
